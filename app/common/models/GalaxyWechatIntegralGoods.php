...
class GalaxyWechatIntegralGoods extends \Phalcon\Mvc\Model
{
...
//        }
     ...
     * @return mixed
     */
    public function getGoodsClassesDeleteData($temp){
        $galaxyWechatIntegralGoodsModel = GalaxyWechatIntegralGoods::class;

        $phql = "SELECT A.galaxy_wechat_classify_id,count(A.is_delete) AS delete_num FROM {$galaxyWechatIntegralGoodsModel} AS A  WHERE A.is_delete = 1 GROUP BY A.galaxy_wechat_classify_id";
        $ret = $this->modelsManager->executeQuery($phql);
        return $ret;
    }

    /**
     * [getErrorResult 联表获取礼品列表]
     * @return [type] [description]
     */
    public function getIntegralGoodsClassesData($params){
        $galaxyWechatIntegralGoodsModel = GalaxyWechatIntegralGoods::class;
        $galaxyWechatClassifyModel = GalaxyWechatClassify::class;

        #0为下架,1为上架,2全部
        $conditions = '';
        if(in_array($params['state'], ['0', '1'])){
            $conditions .= 'AND A.state = '.$params['state'];
        }

        $phql = "SELECT A.id, A.goods_name, A.goods_type, A.galaxy_wechat_classify_id, A.inventory, A.pay_type, A.integral_price, A.state, A.sort, B.classify_name FROM {$galaxyWechatIntegralGoodsModel} AS A LEFT JOIN {$galaxyWechatClassifyModel} AS B ON A.galaxy_wechat_classify_id = B.id WHERE A.is_delete = 0 ".$conditions." ORDER BY A.sort DESC LIMIT ".$params['offset'].",".$params['page_size'];
        $ret = $this->modelsManager->executeQuery($phql);
        return $ret;
    }
