...
class GalaxyWechatClassify extends \Phalcon\Mvc\Model
{
...
//        }
    ...
    /**
     * [getErrorResult 联表获取分类列表]
     * @return [type] [description]
     */
    public function getGoodsClassesData($params){
        $galaxyWechatClassifyModel = GalaxyWechatClassify::class;
        $galaxyWechatIntegralGoodsModel = GalaxyWechatIntegralGoods::class;

        $phql = "SELECT A.id, A.classify_name, A.classify_describe, A.state, A.create_time, A.update_time, A.is_delete,count(B.galaxy_wechat_classify_id) AS classify_num FROM {$galaxyWechatClassifyModel} AS A LEFT JOIN {$galaxyWechatIntegralGoodsModel} AS B ON A.id = B.galaxy_wechat_classify_id WHERE A.is_delete = 0 GROUP BY A.id ORDER BY A.update_time DESC LIMIT ".$params['offset'].",".$params['page_size'];
