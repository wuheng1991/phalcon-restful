...
use Api\Models\GalaxyOrderReveivedRecord;

class GalaxyReorder extends \Phalcon\Mvc\Model
{
...
//        }
    ...
    public function getOrderReveivedRecordData($params, $crm_client_id){
        $galaxyReorderModel = GalaxyReorder::class; #crm订单表
        $galaxyOrderReveivedRecordModel = GalaxyOrderReveivedRecord::class; #crm订单记录表
        #$phql = "SELECT A.projectid, A.orderid, B.fee, B.currency, B.drawee, B.check_status, B.payment_method FROM {$galaxyReorderModel} AS A LEFT JOIN {$galaxyOrderReveivedRecordModel} AS B ON A.id = B.orderid WHERE A.clientid = ".$crm_client_id." AND B.sort = 2 LIMIT ".$params['offset'].",".$params['page_size'];
        $phql = "SELECT A.id, A.fee, A.currency, A.drawee, A.check_status, A.payment_method, B.projectid, B.orderid FROM {$galaxyOrderReveivedRecordModel} AS A LEFT JOIN {$galaxyReorderModel} AS B ON A.orderid = B.id WHERE B.clientid = ".$crm_client_id." AND A.sort = 2 AND A.start_end = 50 LIMIT ".$params['offset'].",".$params['page_size'];
        $ret = $this->modelsManager->executeQuery($phql);
        return $ret;
    }

    public function getOrderReveivedRecordNumData($crm_client_id){
        $galaxyReorderModel = GalaxyReorder::class; #crm订单表
        $galaxyOrderReveivedRecordModel = GalaxyOrderReveivedRecord::class; #crm订单记录表

        $phql = "SELECT count(A.id) AS num FROM {$galaxyOrderReveivedRecordModel} AS A LEFT JOIN {$galaxyReorderModel} AS B ON A.orderid = B.id WHERE B.clientid =".$crm_client_id." AND A.sort = 2 AND A.start_end = 50";
        $ret = $this->modelsManager->executeQuery($phql);
        return $ret;
    }
